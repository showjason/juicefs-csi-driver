...
func parseControllerConfig() {
	...
	if !config.Webhook {
		...
		if err != nil || len(pods) == 0 {
			log.Error(err, "Can't get CSI Node pods with label app=juicefs-csi-node")
			os.Exit(1)
		}
		...
		if csiPod != nil {
			config.CSIPod = *csiPod
			log.Info("Get CSI Node pod successfully", "pod", csiPod.Name)
		} else {
			log.Error(nil, "Can't get CSI Node pods")
			os.Exit(1)
		}
	}
}
